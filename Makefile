PROJECT_NAME := DigitalOcean Package
include build/common.mk

PACK             := digitalocean
PACKDIR          := sdk
PROJECT          := github.com/pulumi/pulumi-digitalocean
...
provider:: generate_schema
	...
	cd provider && go install -ldflags "-X github.com/pulumi/pulumi-digitalocean/provider/v2/pkg/version.Version=${VERSION}" ${PROJECT}/provider/v2/cmd/${PROVIDER}

tfgen::
	cd provider && go install -ldflags "-X github.com/pulumi/pulumi-digitalocean/provider/v2/pkg/version.Version=${VERSION}" ${PROJECT}/provider/v2/cmd/${TFGEN}
